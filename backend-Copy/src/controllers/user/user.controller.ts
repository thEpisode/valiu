function userController (dependencies: any) {
  ...
  const getByIdentity = async (data: any) => {
    try {
  ...
  const getByPhone = async (data: any) => {
    try {
      if (!data || !data.phone) {
        return _utilities.response.error('Please provide a dni')
      }
  ...
  const getByEmail = async (data: any) => {
    try {
      if (!data || !data.email) {
  ...
  const getAllByBusinessId = async (data: any) => {
    try {
