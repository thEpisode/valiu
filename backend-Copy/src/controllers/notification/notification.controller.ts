function notificationController (dependencies: any) {
  const fs = require('fs')
  ...
  const getAllByReceiver = async (data: any) => {
    try {
  ...
  const getAllLastByReceiver = async (data: any) => {
    try {
  ...
  const getAllGroupedByFoldersAndByReceiver = async (data: any) => {
    try {
      const notificationsResponse = await getAllByReceiver(data)
      ...
      for (const notificationCategoryKey in notificationFolders) {
        if (Object.prototype.hasOwnProperty.call(notificationFolders, notificationCategoryKey)) {
          ...
          notificationCategory.notifications = notifications.filter((notification: any) => {
  ...
  const sendStored = async (data: any) => {
    try {
    ...
    try {
      ...
      switch (data.email.template.name) {
        case _models.Notification.email_templates.confirmEmail.name:
          ...
          let emailtemplate: any = await readFileAsync(emailPath)
  ...
  const readFileAsync = async (path: String) => {
    const promise = await new Promise((resolve, reject) => {
      fs.readFile(path, 'utf8', function (err: any, emailTemplate: any) {
        if (err) {
          _console.log(err)
          reject(err)
        } else {
  ...
  const sendEmailAsync = async (transporter: any, mailOptions: any) => {
    const promise = await new Promise((resolve, reject) => {
