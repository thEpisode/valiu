function authController (dependencies: any) {
  ...
  const authenticateUser = async (data: any, user: any) => {
  ...
  const validateEmail = async (data: any) => {
    try {
      ...
      const formatTimestamp:any = new Date(+timestamp)
      const hours = Math.floor(Math.abs(now - formatTimestamp) / 3.6e6)
  ...
  const validateAccountChatbot = async (data: any) => {
