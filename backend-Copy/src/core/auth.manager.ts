class AuthManager {
  ...
  generatePrivateKey (seed: any) {
    ...
    const hashBuffer: any = Uint8Array.from(hashString, (x: String) => x.charCodeAt(0))
    // Cast array buffer to array int
    return [...hashBuffer]
  }

  cypherObject (key: any, payload: any) {
    try {
  ...
  decipherObject (key: any, payload: any) {
    try {
  ...
  createToken (data: any, payload: any) {
    try {
      if (!data || !payload) {
        return null
      }
  ...
  validateToken (token: any) {
    return new Promise((resolve, reject) => {
      this._jwt.verify(token, this._dependencies.config.JWT_SECRET, (err: any, decoded: any) => {
        if (err) {
          return reject(err)
        } else {
          resolve(decoded)
  ...
  async validateApi (req: any, res: any, next: any) {
